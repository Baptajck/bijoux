...
class UserModel {
  static async saveOneUser(req) {
    ...
    return db
      .query(
        'INSERT INTO users (firstName, lastName, email, password, role, address, zip, city, phone, dateCreation, validate, key_id) VALUES( ?, ?, ?, ?, "user", ?, ?, ?, ?, NOW(), "no", ?)',
        [
          req.body.firstName,
          req.body.lastName,
          req.body.email,
    ...
    return db
      ...
      .then((user) => {
        if (user.length === 0) {
          return {
            status: 404,
            msg: "le mail n'existe pas dans la base de donnée",
          };
        }
        return user[0];
      })
  ...
  static async updateValidateUser(key_id) {
    let user = await db.query(
      'UPDATE users SET validate = "yes" WHERE key_id = ?',
      [key_id]
    );
    return user;
  }
  static async updateKeyId(email) {
    let key_id = randomId(len, pattern);
    let user = await db.query("UPDATE users SET key_id = ? WHERE email = ?", [
      key_id,
      email,
    ]);
  ...
  static async updatePassword(newPassword, key_id) {
    ...
    let result = await db.query(
      "UPDATE users SET password = ? WHERE key_id = ?",
      [hash, key_id]
    );
    return result;
  }
