...
module.exports = (app, db) => {
  ...
  app.post("/api/v1/user/forgot", async (req, res, next) => {
    ...
    if (result.code) {
      res.json({
        status: 500,
        msg: "nous n'avons pas pu vous envoyer un email",
        error: result,
      });
    }
    ...
    mail(
      ...
      "Mot de passe oublié ?",
      'Pour modifier votre mot de passe, cliquez <a href="http://localhost:8000/api/v1/user/changePassword/' +
        key_id +
        '">ici<a/> !'
    );
    res.json({
      status: 200,
      msg: "Un email pour réinitialiser votre mot de passe vous a été envoyé",
    });
  });
